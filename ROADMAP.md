...
- [ ] **Location in profile & structured data** — add a `location` field (city/country, translatable) to `PersonalInfo` in [src/home/models.py](src/home/models.py); display it in the hero section near the name/title; include it as `homeLocation` (`@type: Place`) in the Person JSON-LD in [src/home/views.py](src/home/views.py) so Google associates the profile with a specific place (local SEO). _`feat: show location in profile and enrich Person structured data`_
