...
## v0.8.1 — Deployment fixes and improvements ✅ Released

- [x] **Fix canonical domain (www vs non-www)** — add `PREPEND_WWW = env.bool("PREPEND_WWW", default=False)` in [src/core/settings.py](src/core/settings.py); set `PREPEND_WWW=true` in production so Django's `CommonMiddleware` redirects non-www requests to `www.roberfi.com`. Resolves the "Google chose different canonical" warning in Search Console. _`fix: make PREPEND_WWW configurable via env`_
- [x] **Fix request scheme behind proxy** — set `SECURE_PROXY_SSL_HEADER = ("HTTP_X_FORWARDED_PROTO", "https")` when `not DEBUG` in [src/core/settings.py](src/core/settings.py); hardcode `proxy_set_header X-Forwarded-Proto https;` in [deploy/nginx/nginx-proxy.conf](deploy/nginx/nginx-proxy.conf) so Django generates correct `https://` URLs in `og:url`, `og:image`, `twitter:image`, and JSON-LD. _`fix: trust X-Forwarded-Proto in production`_
...
- **Dropped:** "Default language to Spanish" — the language detection is handled by nginx (`map $http_accept_language`) before reaching Django: Spanish browsers (`es*`) are redirected to `/es/`, everything else to `/en/`. `LANGUAGE_CODE = "en-us"` is the correct Django fallback and must not be changed to `es`, as that would serve Spanish to unsupported-language browsers (French, German, Japanese…) that nginx already routed to `/en/`.
